      ...
      <table style={{ width: '100%', borderRadius: '0.5em', borderCollapse: 'collapse', backgroundColor: '#edf4ff' }}>
        ...
        <tbody>
          {here.map((it) => (
            ...
            >
              <td style={{ padding: 15 }}>{it.type === 'folder' ? '📁 folder' : it.type}</td>
              <td style={{ padding: 15, fontWeight: it.type === 'folder' ? 600 : 400 }}>{it.name}</td>
              <td style={{ padding: 15 }}>{'added' in it ? it.added : '—'}</td>
            </tr>
          ))}
        </tbody>
      </table>
